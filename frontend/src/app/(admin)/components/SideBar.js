...
const SideBar = () => {
    ...
    return (
        <div className="w-full h-full sm:h-[100vh] bg-zinc-800 sm:sticky top-0 pt-2 sm:pt-5">
            <div className="w-full flex gap-3 sm:border-b border-zinc-500 pb-2">
                <Bars3CenterLeftIcon className="w-7 h-7 mx-2 mt-1 sm:hidden cursor-pointer text-white" onClick={ () => setShow(true) } />
                <Link href="/admin" className="text-lg flex-grow sm:text-xl md:text-2xl text-white py-1 cursor-pointer px-3">Cool-Market</Link>
            </div>
            <div className={`${ show ? 'block':'hidden' } fixed sm:static sm:block top-0 w-full h-[100vh] sm:h-full z-50 pt-[5%] sm:pt-0 bg-zinc-800 sm:bg-transparent text-white mt-0 sm:mt-4 px-4`}>
                ...
                <Collapse title="Add New">
                    <div className="flex flex-col">
                        <Link 
                            className="text-gray-200 cursor-default text-[15px] pl-3 py-1 hover:text-gray-300 hover:bg-zinc-700 border-b border-zinc-700"
                            href="/admin/create"
                        >Product</Link>
                        <div className="pl-3 py-1">
                            <div className="flex justify-between cursor-default border-b border-zinc-700" onClick={ () => setShowForm(!showForm) }>
                                <span className="text-gray-200 cursor-default text-[15px] py-1 hover:text-gray-300 hover:bg-zinc-700 my-1">Category</span>
                                {
                                    showForm ? (
                                        <ChevronDownIcon className="w-4 h-4 mt-1 text-white" />
                                    ):(
                                        <ChevronRightIcon className="w-4 h-4 mt-2 text-white" />
                                    )
                                }
                            </div>
